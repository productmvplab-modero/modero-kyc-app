...
Deno.serve(async (req) => {
  try {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.setFillColor(...orange);
      pdf.rect(0, footerY - 4, pageWidth, 0.6, 'F');

      // Footer background
      pdf.setFillColor(249, 250, 251);
      pdf.rect(0, footerY - 3.4, pageWidth, 18, 'F');

      // Left: Modero brand
      pdf.setFontSize(7);
      pdf.setFont(undefined, 'bold');
      pdf.setTextColor(...orange);
      pdf.text('Modero', margin, footerY + 3);

      pdf.setFont(undefined, 'normal');
      pdf.setTextColor(...midGray);
      pdf.text(' · www.moderokyc.com · This document is legally binding once signed by all parties.', margin + 14, footerY + 3);

      // Center: timestamp
      pdf.setFont(undefined, 'normal');
      pdf.setTextColor(...midGray);
      pdf.text(`Generated: ${timestamp}`, pageWidth / 2, footerY + 8, { align: 'center' });

      // Right: page number
      pdf.setFont(undefined, 'bold');
      pdf.setTextColor(...orange);
      pdf.text(`${i}`, pageWidth - margin, footerY + 3, { align: 'right' });
      pdf.setFont(undefined, 'normal');
      pdf.setTextColor(...midGray);
      pdf.text(`/ ${totalPages}`, pageWidth - margin + 1, footerY + 3);
    }
