...
export default function TenantDetailsDialog({ inquiry, open, onOpenChange, properties = [], onOpenProperty }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[95vh] overflow-y-auto w-[95vw] sm:w-auto [&>button]:p-2 sm:[&>button]:p-3 [&>button]:rounded-lg sm:[&>button]:rounded-xl [&>button]:right-4 sm:[&>button]:right-6 [&>button]:top-8 sm:[&>button]:top-9 [&>button]:hover:bg-slate-100 [&>button>svg]:h-4 sm:[&>button>svg]:h-5 [&>button>svg]:w-4 sm:[&>button>svg]:w-5">
        <DialogHeader>
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          {/* Landlord Decision */}
          <Card className="bg-gradient-to-r from-blue-50 to-slate-50 border-blue-200 overflow-hidden border-0 shadow-md">
            <div className="h-1 bg-gradient-to-r from-blue-500 via-blue-400 to-slate-300" />
            <CardContent className="pt-4 sm:pt-6 px-3 sm:px-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-base sm:text-lg text-slate-900">{t('landlord_decision')}</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                {[
                  ...
                  <div key={name} className="flex items-center gap-3 p-3 rounded-lg border border-slate-200 bg-slate-50">
                    <div className={['h-8 w-8 sm:h-9 sm:w-9 rounded-lg flex items-center justify-center text-white text-xs font-bold shrink-0', bg].join(' ')}>
                        {logo}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs sm:text-sm font-medium text-slate-900">{name}</p>
                        <p className="text-xs text-slate-500 line-clamp-1">{provider}</p>
                    </div>
