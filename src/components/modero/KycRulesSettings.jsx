...
export default function KycRulesSettings({ userEmail }) {
  ...
  return (
    <div>
      ...
      {/* Credit */}
      <Section icon={Gauge} title="Creditworthiness" description="Set credit score and check requirements">
      ...
      <div className="flex justify-end mt-6">
        <Button onClick={() => saveMutation.mutate(rules)} disabled={saveMutation.isPending} className="gap-2 bg-orange-500 hover:bg-orange-600 px-8">
          {saveMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
          Save All Rules
        </Button>
      </div>
