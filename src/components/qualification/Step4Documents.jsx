...
function DocUploader({ docType, documents, onUpload, t }) {
        ...
        <div className="flex-1 min-w-0">
          ...
          {url && (
            <a href={url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-green-600 mt-1 hover:underline">
              <ExternalLink className="w-3 h-3" /> {t('s4_view')}
            </a>
          )}
        </div>
...
export default function Step4Documents({ formData, updateForm, onNext, onBack, t }) {
  const [loading, setLoading] = useState(false);

  const docTypes = [
    { key: 'cv_url', label: t('s4_cv'), desc: t('s4_cv_desc'), required: true, icon: '📄' },
    { key: 'payslip_url', label: t('s4_payslip'), desc: t('s4_payslip_desc'), required: true, icon: '💰' },
    { key: 'work_contract_url', label: t('s4_contract'), desc: t('s4_contract_desc'), required: false, icon: '📋' },
  ...
  return (
    ...
    >
      <div className="bg-orange-50 border border-orange-200 rounded-xl p-3 flex items-start gap-2">
        ...
        <p className="text-xs text-orange-700">{t('s4_security')}</p>
      </div>

      <div className="space-y-3">
        {docTypes.map(doc => (
          <DocUploader key={doc.key} docType={doc} documents={formData.documents} onUpload={handleUpload} t={t} />
        ))}
      </div>

      <div className="text-xs text-slate-400 text-center">
        {requiredDocs.filter(d => formData.documents[d.key]).length} / {requiredDocs.length} {t('s4_count')}
      </div>
    </StepCard>
  );
}