...
import { Linkedin, Facebook, Mail, Phone, CheckCircle2, ChevronDown } from 'lucide-react';
...
export default function Step1Profile({ formData, updateForm, onNext }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold text-orange-600">Welcome! Begin your application process</h1>
        <p className="text-slate-500 text-sm mt-2">You are just 8 steps away from qualifying for your apartment.</p>
      </div>
      ...
      <div className="space-y-3 mb-6">
        ...
        >
          ...
          {formData.linkedin_connected ? 'LinkedIn Connected ✓' : 'Continue with LinkedIn'}
        </button>
        <button
        ...
        >
          ...
          {formData.xing_connected ? 'XING Connected ✓' : 'Continue with XING'}
        </button>
        <button
        ...
        >
          ...
          {formData.facebook_connected ? 'Facebook Connected ✓' : 'Continue with Facebook'}
        </button>
      </div>
      ...
      <div className="relative mb-6">
        ...
        <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-4 text-xs text-slate-400 font-medium">OR</span>
      </div>
      ...
      <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              >
                {verifying ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> :
                  formData.mobile_verified ? <><CheckCircle2 className="w-3.5 h-3.5" /> Verified</> : 'Verify'}
              </button>
            </div>
          </div>
        </div>
        ...
        >
          {loading ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <>Continue to Qualification <ArrowRight className="w-4 h-4" /></>}
        </Button>
        <p className="text-xs text-slate-400 text-center mt-2">We'll guide you through the qualification process step by step</p>
      </div>
    </div>
  );
}